...
def xls_to_csv(path, destination, file):
	...
	print(f'converting excel file with {len(sheets)} sheets into a csv file with 1 sheet')

	if len(sheets) == 1:
	    xls.parse(sheets[0]).to_csv(destination + file[:-5] + '.csv', encoding = 'utf-8')
	else:
	    ...
	    final_df.to_csv(destination + file[:-5] + '.csv', encoding = 'utf-8')
	print('done')
